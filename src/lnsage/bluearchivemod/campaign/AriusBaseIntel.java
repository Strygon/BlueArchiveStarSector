...
public class AriusBaseIntel extends BaseIntelPlugin implements EveryFrameScript, FleetEventListener,
	...
	public static Logger log = Global.getLogger(AriusBaseIntel.class);
	...
	public static AriusBaseIntel getIntelFor(StarSystemAPI system) {
		for (IntelInfoPlugin intel : Global.getSector().getIntelManager().getIntel(AriusBaseIntel.class)) {
			if (((AriusBaseIntel)intel).getSystem() == system) {
				return (AriusBaseIntel) intel;
			}
		}
		return null;
	}
	
	public static AriusBaseIntel getIntelFor(MarketAPI market) {
		for (IntelInfoPlugin p : Global.getSector().getIntelManager().getIntel(AriusBaseIntel.class)) {
			AriusBaseIntel intel = (AriusBaseIntel) p;
			if (intel.getMarket() == market) return intel;
		}
		return null;
	}
	
	public AriusBaseIntel(StarSystemAPI system, String factionId) {
		this.system = system;
	
		market = Global.getFactory().createMarket(Misc.genUID(), "Arius Base", 3);
		market.setSize(3);
		...
		market.setFactionId(Factions.ARIUS);
		...
		log.info(String.format("Added arius base in [%s], isLarge: %s", system.getName(), "" + large));
	}
...
//		entity.getDetectedRangeMod().unmodify("gen");
	...
	protected void notifyEnding() {
		super.notifyEnding();
		log.info(String.format("Removing arius base at [%s]", system.getName()));
	...
	public void reportFleetDespawnedToListener(CampaignFleetAPI fleet, FleetDespawnReason reason, Object param) {
		...
		if (addedListenerTo != null && fleet == addedListenerTo) {
			...
			for (AriusCellsIntel cell : AriusCellsIntel.getCellsForBase(this, false)) {
				...
				if (cell.getMarket().isPlayerOwned() || DebugFlags.PATHER_BASE_DEBUG) {
					cell.sendUpdateIfPlayerHasIntel(AriusCellsIntel.UPDATE_DISRUPTED, false);
				}
				// ListenerUtil.reportCellDisrupted(cell);
			}
			AriusBaseManager.getInstance().incrDestroyed();
...
//		}
		...
		for (AriusCellsIntel cell : AriusCellsIntel.getCellsForBase(this, true)) {
