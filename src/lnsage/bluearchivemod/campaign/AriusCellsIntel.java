...
import lnsage.bluearchivemod.campaign.ids.Conditions;
import lnsage.bluearchivemod.campaign.ids.Factions;

public class AriusCellsIntel extends BaseIntelPlugin implements RouteFleetSpawner, FleetEventListener {
...
//	public static float DISRUPTION_RANGE  = 60;
	...
	public AriusCellsIntel(MarketAPI market, boolean sleeper) {
		this.market = market;
		...
		if (!market.hasCondition(Conditions.ARIUS_CELLS)) {
			market.addCondition(Conditions.ARIUS_CELLS, this);
		}
	...
	public static AriusBaseIntel getClosestBase(MarketAPI market) {
		List<IntelInfoPlugin> bases = Global.getSector().getIntelManager().getIntel(AriusBaseIntel.class);
		float minDist = Float.MAX_VALUE;
		AriusBaseIntel closest = null;
		for (IntelInfoPlugin curr : bases) {
			AriusBaseIntel intel = (AriusBaseIntel) curr;
	...
	public static List<AriusCellsIntel> getCellsForBase(AriusBaseIntel base, boolean includeSleeper) {
		List<AriusCellsIntel> result = new ArrayList<AriusCellsIntel>();
		
		List<IntelInfoPlugin> cells = Global.getSector().getIntelManager().getIntel(AriusCellsIntel.class);
		for (IntelInfoPlugin curr : cells) {
			AriusCellsIntel intel = (AriusCellsIntel) curr;
	...
	public static AriusCellsIntel getCellsForMarket(MarketAPI market) {
	...
	protected void notifyEnding() {
		...
		if (market.hasCondition(Conditions.ARIUS_CELLS)) {
			market.removeCondition(Conditions.ARIUS_CELLS);
		}
	}
...
//					}
	...
	public void addInterestInfo(TooltipMakerAPI info, float width, float height) {
		...
		if (aiCoreId != null) {
			int s = (int) Math.round(AriusBaseManager.AI_CORE_ADMIN_INTEREST);
	...
	public void createSmallDescription(TooltipMakerAPI info, float width, float height) {
		...
		if (isEnding()) {
			info.addPara("The Arius cells " +
					market.getOnOrAt() + " " + market.getName() + " have been dissolved.", opad);
		} else if (isSleeper() && sleeperTimeout <= 0) {
			info.addPara("There are indications that sleeper Arius cells are being organized " +
					market.getOnOrAt() + " " + market.getName() + ".", opad);
			info.addPara("Arius have not made any significant moves, but are apparently preparing " +
					"to do so if given the opportunity and material to do so and " +
					"no further actions are being taken to stop them.", opad);
		} else {
			info.addPara("There are active Arius cells " +
					market.getOnOrAt() + " " + market.getName() + ".", opad);
...
//					+ " The cells also provide intel to Pather fleets operating in-system.", opad);
			info.addPara("They are engaged in planning acts of terror and industrial sabotage, but " +
					...
					+ " The cells also provide intel to Arius fleets operating in-system.", opad);
			...
			if (sleeperTimeout > 0) {
				...
				AriusBaseIntel base = AriusCellsIntel.getClosestBase(market);
				if (base != null) {
					if (base.isPlayerVisible()) {
						info.addPara("The Arius base at the " + base.getMarket().getStarSystem().getNameWithLowercaseType() +
							" is providing support to these cells.", opad);
					} else {
						info.addPara("You do not know the location of the Arius base providing support to these cells.", opad);
					}
		...
		if (!isEnding()) {
			...
			if (!isSleeper()) {
				float stability = AriusCells.STABLITY_PENALTY;
...
//						"-" + (int)stability);
		...
		if (prevIncident != null || incidentType == IncidentType.PLANETBUSTER) {
			...
			if (prevIncidentSucceeded) {
				if (incidentType == IncidentType.PLANETBUSTER) {
					info.addPara("There are indications that Arius cells are preparing to sneak " +
					...
					case INDUSTRY_SABOTAGE:
						if (prevIncidentData instanceof Industry) {
							...
							if (ind.getDisruptedDays() > 2) {
								String days = getDays(ind.getDisruptedDays());
								String daysStr = getDaysString(ind.getDisruptedDays());
								info.addPara("Arius cells have conducted a successful act of sabotage, " +
						...
						break;
					case REDUCED_STABILITY:
						info.addPara("Arius cells have conducted low-level attacks on various " +
								"industrial, military, and civilian targets, reducing stability by %s.",
								opad, h, "" + (Integer) prevIncidentData);
						break;
					case PLANETBUSTER:
						info.addPara("Arius cells have smuggled a planetbuster onto " + market.getName() +
	...
	public String getIcon() {
		if (isSleeper()) {
			return Global.getSettings().getSpriteName("intel", "arius_cells_inactive");
		}
		return Global.getSettings().getSpriteName("intel", "arius_cells_active");
	}
	...
	public FactionAPI getFactionForUIColors() {
		return Global.getSector().getFaction(Factions.ARIUS);
	}
	...
	public void prepareIncident() {
		...
		AriusBaseIntel base = AriusCellsIntel.getClosestBase(getMarket());
		if (base == null) return;
...
//		}
	...
	protected void sendSmuggler(AriusBaseIntel base) {
